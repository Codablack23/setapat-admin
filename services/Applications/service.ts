...
import { AcceptApplicationDto, DesignerApplication, GetApplicationsResponse } from "./types";
...
export class ApplicationsProvider{
    ...
    static getApplicationDetails(id:string,){
        return ApiProvider.get<{application:DesignerApplication}>(`${this.path}/${id}`)
    }
    static acceptApplication(id:string,dto:AcceptApplicationDto){
        return ApiProvider.post(`${this.path}/${id}/accept`,dto)
    }
